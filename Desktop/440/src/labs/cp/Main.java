...
public class Main
    ...
    public static Model initQFunction() {
        ...
        // predict
        // one q-value per (fixed ahead of time) actions. In this world there are two
        // actions.
    ...
    public static int argmax(Matrix qValues) throws IndexOutOfBoundsException {
        ...
        // small subtlety is that in the unexpected case where our neural network starts
        // spitting
        // out NaNs or Infs or something we won't get an answer if we start off maxVal
        // at -inf
        // so instead I like to start it off at null, just to guarantee that the first
        // element gets picked
            ...
            Namespace ns) // namespace of command line arguments
    {
        ...
        for (int gameIdx = 0; gameIdx < numTrainingGames; ++gameIdx) {
            // TODO: complete me!
            // play a bunch of training games where you are not allowed to update the neural
            // network
            // make sure to add transitions that you observe to the replay buffer, including
            ...
            while (!isDone) {
                int action;
                if (game.getRandom().nextDouble() < epsilon) {
                    action = game.getRandom().nextInt(2);
                } else {
                    try {
                        Matrix qvalues = qFunction.forward(state);
                        int qMax = argmax(qvalues);
                        action = qMax;
                ...
                Triple<Matrix, Double, Boolean> status = game.step(action);
                if (status.third() == true) {
                    rb.addSample(state, status.second(), null);
                    isDone = true;
                } else {
                    rb.addSample(state, status.second(), status.first());
                }
                state = status.first();
            ...
            Namespace ns) // namespace of command line args
    {
        ...
        for (int epochIdx = 0; epochIdx < numUpdates; ++epochIdx) {
            ...
            // shuffle the training data to prevent the network from thinking some examples
            // belong by others
            ds.shuffle();
    ...
    public static Pair<Double, Double> test(Game game, Model qFunction, Namespace ns) {
        ...
        for (int gameIdx = 0; gameIdx < numEvalGames; ++gameIdx) {
            ...
            int t = 0;
            while (!isDone) {
                // ask the q function to rank each action and then listen to it (i.e. argmax for
                // policy)
                try {
                    Matrix qValues = qFunction.forward(state);
    ...
    public static void main(String[] args) {
        ...
        // but if you change it to Adam be sure to include the command line arguments
        // for Adam!
        ...
        for (int cycleIdx = 0; cycleIdx < numCycles; ++cycleIdx) {
            ...
            // update the model by converting the replay buffer into a supervised learning
            // dataset and doing gd
            ...
            System.out.println(
                    "after cycle=" + cycleIdx + " avg(utility)=" + avgUtil + " avg(game_length)=" + avgGameLength);
        }
    }
}
