...
public class ReplayBuffer
    ...
    public void addSample(Matrix prevState,
        ...
        int capacity = this.prevStates.getShape().numRows();

        if (currentSize < capacity) {
            this.setNewestSampleIdx(currentSize);
            this.setSize(currentSize + 1);
            this.prevStates.copySlice(currentSize, currentSize + 1, 0, this.prevStates.getShape().numCols(), prevState);
            this.rewards.set(currentSize, 0, reward);
            if (nextState != null) {
                this.nextStates.copySlice(currentSize, currentSize + 1, 0, this.nextStates.getShape().numCols(),
                        nextState);
            }
            this.isStateTerminalMask[currentSize] = nextState == null;
        } else {
            int idxToEvict = this.chooseSampleToEvict();
            this.prevStates.copySlice(idxToEvict, idxToEvict + 1, 0, this.prevStates.getShape().numCols(), prevState);
            this.rewards.set(idxToEvict, 0, reward);
            if (nextState != null) {
                this.nextStates.copySlice(idxToEvict, idxToEvict + 1, 0, this.nextStates.getShape().numCols(),
                        nextState);
            }
            this.isStateTerminalMask[idxToEvict] = nextState == null;
            this.setNewestSampleIdx(idxToEvict);
    ...
    public Matrix getGroundTruth(Model qFunction,
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (isStateTerminalMask[i] == true) {
                target = reward;
            } else {
                try {
                    Matrix sPrime = nextStates.getRow(i);
                    Matrix qValues = qFunction.forward(sPrime);
                    double qMax = max(qValues);
                    target = reward + (discountFactor * qMax);
                } catch (Exception e) {
                    System.err.println("OH OHH something wrong here with the qFunction");
                    System.exit(-1);
                }
            }
